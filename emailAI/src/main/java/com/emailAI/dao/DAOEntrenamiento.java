...
public class DAOEntrenamiento {
    ...
    public static class Ejemplo {
        ...
        public Ejemplo(String remitente, String asunto, String cuerpo, String etiqueta, String tipoModelo) {
    ...
    private void crearTablaSiNoExiste() throws SQLException {
        // Añadimos 'tipo_modelo' para no mezclar datos de entrenamiento
        String sql = """
    ...
    public void guardarEjemplo(Mensaje mensaje, String etiqueta, String tipoModelo) throws SQLException {
        String sql = "INSERT INTO ejemplos_entrenamiento(remitente, asunto, cuerpo, etiqueta, tipo_modelo) VALUES (?, ?, ?, ?, ?)";
        try (Connection conn = ConexionBD.getConnection();
    ...
    // Método clave para el atributo 'es_remitente_frecuente'
    public int contarInteracciones(String remitente) throws SQLException {
        String sql = "SELECT COUNT(*) FROM ejemplos_entrenamiento WHERE remitente = ? AND etiqueta != 'SPAM'";
        try (Connection conn = ConexionBD.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public List<Ejemplo> listarEjemplosPorTipo(String tipo) throws SQLException {
        String sql = "SELECT remitente, asunto, cuerpo, etiqueta, tipo_modelo FROM ejemplos_entrenamiento WHERE tipo_modelo = ?";
        ...
        try (Connection conn = ConexionBD.getConnection();
            ...
            while (rs.next()) {
                lista.add(new Ejemplo(
                        rs.getString("remitente"), rs.getString("asunto"),
                        rs.getString("cuerpo"), rs.getString("etiqueta"),
